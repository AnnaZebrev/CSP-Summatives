...
def draw_leaderboard(high_scorer, leader_names, leader_scores, turtle_object, player_score):
    ...
    for index in range(len(leader_names)):
        turtle_object.turtlesize(1)
        turtle_object.shape("circle")
        if leader_scores[index] >= gold_score:
            turtle_object.color("gold")
            turtle_object.stamp()
        elif leader_scores[index] >= silver_score:
            turtle_object.color("silver")
            turtle_object.stamp()
        elif leader_scores[index] >= bronze_score:
